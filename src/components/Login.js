    ...
    <div className="h-full w-full flex items-center justify-center select-none">
      ...
      <div className="">
        <div className="fixed right-6 md:right-4 top-4 items-center justify-center space-x-2 hidden md:flex">
          <div className="relative">
            {user ? <HiUserAdd onClick={(e) => {setShowUserAdd(!showUserAdd)}} className='text-slate-600 hover:text-slate-700 text-xl cursor-pointer' /> : ''}
            {showUserAdd ? <form onSubmit={onUserAdd} className="absolute top-[115%] right-0 px-6 py-8 w-max rounded-lg text-slate-600 bg-slate-100 ring-1 ring-slate-300">
              <p className="mb-0.5 text-xs pl-1 font-bold">Név</p>
              <input required value={userName} onChange={(e) => setUserName(e.target.value)} placeholder="Név" type="text" className={`w-full bg-slate-200 rounded-lg outline-none px-3 py-1 placeholder:text-sm`} />
              <p className="mt-3 text-xs pl-1 font-bold">E-mail</p>
              <input required value={userMail} onChange={(e) => setUserMail(e.target.value)} placeholder="E-mail" type="email" className={`w-full bg-slate-200 rounded-lg outline-none px-3 py-1 placeholder:text-sm`} />
              <button className="w-full bg-emerald-500 hover:bg-emerald-600 text-white py-1 rounded-lg mt-3">Tovább</button>
            </form> : ''}
          </div>
          {user ? <HiLogin onClick={() => signOut(auth)} className='text-slate-600 hover:text-slate-700 text-xl cursor-pointer' /> : <HiIdentification onClick={() => setShowLogin(true)} className=' text-slate-600 hover:text-slate-700 text-xl cursor-pointer'/>}
        </div>
        {showLogin ? <div className="fixed top-0 left-0 bg-slate-500 bg-opacity-60 w-full h-full z-10 flex items-center justify-center">
          <form onSubmit={onAdminLogin} action="" className='relative text-slate-600 bg-slate-100 px-6 py-8 rounded-lg'>
            ...
            {errorMessage !== '' && code === '' ?
            <p className="text-pink-500 mt-0.5 text-xs pl-3">{errorMessage}</p> : ''
            }
            <p className="mt-3 mb-0.5 text-xs pl-1 font-bold">Jelszó</p>
            <input required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Jelszó" type="password" className={`w-full bg-slate-200 rounded-lg outline-none px-3 py-1 placeholder:text-sm`} />
            <button className="w-full bg-emerald-500 hover:bg-emerald-600 text-white py-1 rounded-lg mt-3">Tovább</button>
          </form>
        </div> : ''}
      <form onSubmit={onLogin} action="" className="text-slate-600">
        ...
        <div className="">
          ...
          <input required value={code} onChange={(e) => setCode(e.target.value)} placeholder="RÖszTI Kód" type="text" className={`w-full bg-slate-200 rounded-lg outline-none px-3 py-1 placeholder:text-sm ${errorMessage !== '' && code === '' ? 'ring-1 ring-pink-500 text-pink-500' : ''}`} />
          {errorMessage !== '' && code === '' ?
            <p className="text-pink-500 mt-0.5 text-xs pl-3">{errorMessage}</p> : ''
          }
        </div>
        <button className="w-full bg-emerald-500 hover:bg-emerald-600 text-white py-1 rounded-lg mt-2">Tovább</button>
      </form>
      </div>}
    </div>
  )
